...
class SectionPromotionLockingTests(TestCase):
    ...
    def test_promote_locks_section(self):
        ...
        self.assertEqual(self.section.approved_content, 'Reviewed draft')
        ...
        detail = client.get(f'/api/proposals/{self.proposal.id}/')
        section_data = detail.json()['sections'][0]
        self.assertEqual(section_data['state'], 'approved')
        self.assertEqual(section_data['approved_content'], 'Reviewed draft')
        self.assertEqual(section_data['draft_content'], 'Reviewed draft')
        self.assertTrue(section_data['locked'])
