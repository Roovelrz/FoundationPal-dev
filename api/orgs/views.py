...
class OrganizationViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return Organization.objects.filter(memberships__user=user).distinct().order_by('created_at', 'id')
    ...
    def perform_create(self, serializer):
        ...
        with transaction.atomic():
            ...
            if not requested_name:
                next_number = Organization.objects.filter(
                    memberships__user=user
                ).distinct().count() + 1
                requested_name = f'工作区 {next_number}'
            org = serializer.save(admin=user, name=requested_name)
